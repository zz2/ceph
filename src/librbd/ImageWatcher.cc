...
int ImageWatcher::register_watch() {
  ...
  m_watch_error = m_image_ctx.md_ctx.watch(m_image_ctx.header_oid, &m_handle,
				           &m_watch_ctx);
...
int ImageWatcher::unregister_watch() {
  ...
  RWLock::WLocker l(m_watch_lock);
  return m_image_ctx.md_ctx.unwatch(m_handle);
}
...
int ImageWatcher::lock() {
  ...
  FunctionContext *ctx = new FunctionContext(
    boost::bind(&IoCtx::notify, &m_image_ctx.md_ctx, m_image_ctx.header_oid,
...
int ImageWatcher::notify_async_progress(const RemoteAsyncRequest &request,
  ...
  m_image_ctx.md_ctx.notify(m_image_ctx.header_oid, bl, NOTIFY_TIMEOUT, NULL);
  return 0;
}
				        ...
				        const std::string &oid)
{
  ...
  io_ctx.notify(oid, bl, NOTIFY_TIMEOUT, NULL);
}
...
int ImageWatcher::notify_lock_owner(bufferlist &bl, bufferlist& response) {
  ...
  int r = m_image_ctx.md_ctx.notify(m_image_ctx.header_oid, bl, NOTIFY_TIMEOUT,
				    &response_bl);
  if (r < 0 && r != -ETIMEDOUT) {
...
void ImageWatcher::reregister_watch() {
...

  {
    ...
    {
      RWLock::WLocker l(m_watch_lock);
      m_image_ctx.md_ctx.unwatch(m_handle);
      m_watch_error = m_image_ctx.md_ctx.watch(m_image_ctx.header_oid,
