...
public:
  ...
  public:
    ...
    int watch(const librbd::ImageCtx &ictx) {
      ...
      return m_parent.m_ioctx.watch(m_header_oid, &m_handle, this);
    }

    int unwatch() {
      return m_parent.m_ioctx.unwatch(m_handle);
    }
...
TEST_F(TestImageWatcher, RequestLock) {
  ...
  ASSERT_EQ(0, m_ioctx.notify(ictx->header_oid, bl, 5000, NULL));
