...
void TestIoCtxImpl::aio_flush_async(AioCompletionImpl *c) {
  m_client->add_aio_operation(boost::bind(&TestIoCtxImpl::aio_flush, this), c);
}

int TestIoCtxImpl::aio_operate(const std::string& oid, ObjectOperations &ops,
                               AioCompletionImpl *c, SnapContext *snap_context,
                               int flags) {
  // TODO ignoring snap_context and flags for now
  m_client->add_aio_operation(boost::bind(
...
int TestIoCtxImpl::operate(const std::string& oid, ObjectOperations &ops) {
...
int TestIoCtxImpl::operate_read(const std::string& oid, ObjectOperations &ops,
                                bufferlist *pbl) {
  AioCompletionImpl *comp = new AioCompletionImpl();
...
int TestIoCtxImpl::execute_aio_operations(const std::string& oid,
                                          ObjectOperations &ops,
                                          bufferlist *pbl) {
  for (ObjectOperations::iterator it = ops.begin(); it != ops.end(); ++it) {
    int ret = (*it)(this, oid, pbl);
    if (ret < 0) {
      return ret;
    }
  }
